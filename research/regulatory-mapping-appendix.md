...
- locate, for a given regulatory obligation, the Clad control that addresses it and the chapter where it is defined;
- distinguish what the framework provides from what remains an organizational responsibility;
- use the crosswalk to prepare audit evidence without overclaiming coverage.

**A necessary disclaimer.** This mapping shows where the framework provides relevant controls. It does **not** claim that deploying the framework achieves full compliance with any regulation. Compliance requires organizational controls, policies, training, and regulatory engagement beyond what any technical framework provides. The coverage ratings reflect architectural design intent; as noted in the Preface, the framework is operationally unverified, and effectiveness must be confirmed empirically.
...
| GOV-3: Workforce diversity and training | MDR §7.1 (staffing, runbook) | Partial | MDR defines staffing model and runbook requirement. Broader workforce training is organizational, not framework-provided. |
...
| NIST AI RMF | Solution Component | Coverage | Notes |
|---|---|---|---|
| MEA-1: Risk metrics | ROC §8.2 (classifier governance), MDR §3.2 (drift detection) | Moderate | Classifier precision/recall/ECE, constraint violation rates, drift metrics. Not a complete risk measurement framework. |
| MEA-2: AI system performance | ROC §4.2 (classifier profiles), MDR §3.3 (pattern detection) | Moderate | Classifier performance tracked. Model performance per se is out of scope. |
...
| NIST AI RMF | Solution Component | Coverage | Notes |
|---|---|---|---|
| MAN-1: Risk treatment | EPG (prompt constraints), ROC (output controls), MDR (monitoring) | Strong | Three-component governance pipeline treats prompt, output, and monitoring risks. |
| MAN-2: Risk response | MDR §5 (incident response workflows) | Strong | Containment, investigation, remediation, reporting workflows defined. |
| MAN-3: Risk communication | MDR §5.4 (reporting), EPG §9.6 (limitations) | Moderate | Internal and regulatory reporting. Stakeholder communication is organizational. |
| MAN-4: Risk monitoring | MDR §3 (cross-component monitoring), §4 (alert taxonomy) | Strong | Continuous monitoring with tiered alerting and escalation. |
...
|---|---|---|---|---|
| Risk management system | Art. 9 | Meta-framework (risk model §10), EPG, ROC, MDR | Strong | Three-component system with formal risk model, residual risk documentation, and continuous monitoring. |
...
| Technical documentation | Art. 11 | All chapters (meta-framework, EPG, ROC, MDR) | Strong | Comprehensive technical documentation of governance architecture, formal models, and operational procedures. |
| Record-keeping and logging | Art. 12 | Meta-framework §9 (audit chain), EPG Appendix A.1.12, ROC Appendix A.2.4, MDR §3.1 | Strong | Tamper-evident, version-stamped audit records with full chain composition across components. GIL provides interaction-level traceability. |
| Transparency | Art. 13 | EPG §9.6 (limitations), ROC §9 (limitations), MDR §8 (limitations) | Moderate | Each component explicitly states what it guarantees and what it does not. User-facing transparency (informing users they are interacting with AI) is organizational, not framework-provided. |
| Human oversight | Art. 14 | EPG §4.5 (break-glass), §5.6 (HITL trigger), ROC §5.4 (escalation), MDR §5 (incident response) | Moderate | Human-in-the-loop review for residual gaps, escalation for blocked outputs, human incident response. Routine human oversight of AI decisions is not mandated by the framework. |
| Accuracy, robustness, cybersecurity | Art. 15 | ROC §4.2 (classifier governance), §8.2 (validation), MDR §3.2 (drift detection) | Partial | Classifier accuracy tracked. Robustness via red-team testing. Cybersecurity of the AI system itself is infrastructure-level, partially addressed by MDR containment. Model accuracy is out of scope. |
...
| Post-market monitoring | Art. 72 | MDR §3 (monitoring), §5.4 (trend reporting) | Strong | Continuous monitoring, drift detection, trend reporting. |
...
| 7. Support (resources, competence) | MDR §7.1 (staffing), EPG §9.5 (tooling) | Moderate | Staffing model and tooling recommendations defined. Training and awareness are organizational. |
...
| 9. Performance evaluation | MDR §3 (monitoring), §5.4 (reporting), ROC §8.2 (classifier governance) | Strong | Continuous monitoring, trend reporting, classifier performance tracking. |
| 10. Improvement | MDR §5.3 (remediation), EPG §5.7 (decomposition verification) | Moderate | Remediation workflows and periodic re-validation. Continual improvement process is organizational. |
...
|---|---|---|---|
| Internal controls documentation | All chapters (meta-framework, EPG, ROC, MDR) | Strong | Comprehensive documentation of governance controls, formal models, and operational procedures. |
| Segregation of duties | EPG §4.2 (orthogonal dual control) | Moderate | Author ≠ approver with different reporting lines for Critical constraints. Central non-business-unit approver required. Mitigates single-actor and same-unit abuse; does NOT prevent cross-unit collusion (EPG §4.2 collusion limitation). |
...
| Management assessment of controls | MDR §5.4 (trend reporting), ROC §8.2 (classifier governance) | Moderate | Governance health metrics, classifier performance tracking, and trend reporting support management assessment. Formal control testing is organizational. |
...
|---|---|---|---|
| Safeguards Rule — information security program | Meta-framework (governance architecture), EPG, ROC, MDR | Strong | Three-component governance with formal controls, monitoring, and incident response. |
...
| Incident response | MDR §5 (incident response) | Strong | Containment, investigation, remediation, reporting workflows with forensic evidence locker. |
...
| CIP-004: Personnel and training | MDR §7.1 (staffing, runbook) | Partial | Staffing model and runbook defined. Formal personnel security screening is organizational. |
...
| CIP-007: System security management | EPG §9.3 (change management), MDR §3.2 (config drift) | Strong | Change management with staging and rollback. Configuration drift detection. |
| CIP-008: Incident reporting and response | MDR §5 (incident response), §5.4 (forensic evidence locker) | Strong | Incident detection, containment, investigation, and reporting workflows with evidence preservation. |
| CIP-011: Information protection | ROC §6.3 (PII/PHI filtering), EPG: F(operational_commands_without_confirmation) | Moderate | Output filtering for sensitive data (classifier-based, probabilistic). Safety constraints preventing unauthorized operational commands (deterministic). Cross-tenant isolation (T11) is monitoring-only, not a preventative control (ROC §6.4). |
...
| EPG (Chapter 2) | Govern, Manage | Art. 9, 14, 17 | Cl. 5, 6, 8 | §164.502, 520 | Controls, SoD | Data protection | CIP-003, 007 |
| ROC (Chapter 3) | Measure, Manage | Art. 9, 15 | Cl. 9 | §164.502(a) | — | Data protection | CIP-011 |
| MDR (Chapter 4) | Manage | Art. 72 | Cl. 9, 10 | §164.404 | Assessment | Incident response | CIP-008 |
...
- The framework provides *relevant controls* against every standard surveyed, but coverage is uneven: strongest on record-keeping, risk treatment, and monitoring; partial or out of scope on data governance, model internals, and anything that is fundamentally an organizational responsibility.
- The recurring boundary is technical vs. organizational. Clad can produce the evidence a regulator asks for; it cannot substitute for the policies, training, and human processes that surround it.
- Used honestly — as a map from obligation to control to chapter — the crosswalk shortens audit preparation without overclaiming compliance.

This is where the book's argument lands: governance that is *provable* gives an enterprise something to show a regulator. The formal model behind these claims, the worked examples, the reusable templates, and a glossary of terms and notation follow in the appendices.