...
\paragraph{Labelling and adjudication.} Authenticity and versioning can be determined
mechanically from metadata. Authority, applicability, support, sufficiency, and defeat
require judgement, and therefore require raters with domain competence, a written
...
\paragraph{The instrument has its own error rate.} If applicability is labelled by a
...
\subsection{Estimating evidence dependence, and why naive ablation fails}
\label{sec:sensitivity-protocol}

The sensitivity condition of Definition~\ref{def:grounding} is the one measurement in
this paper that requires a designed intervention rather than an inspection, and the
...
behaviour moves under semantically inert input variation. So a changed assertion after
...
in both directions by the paper's own central argument.
...
and measurable only by designed intervention for the dependence condition. A programme
...
The measurement is cheap---no raters, no answer key---and it directly quantifies how much
