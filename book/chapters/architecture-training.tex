% !TEX root = ../main.tex
\chapter{The Systems in Scope}
\label{ch:architecture-training}
...
of one architecture and one training regime, and it does not generalize to deep
neural networks at large.

\section{What is in scope, and what is not}
\label{sec:scope}
...
preference optimization \citep{ouyang2022}, and everything built on that pattern
...
\begin{itemize}
  \item \textbf{Generative adversarial networks} and \textbf{diffusion models}.
        ...
        organization relies on as an assertion. A synthesized image makes no claim
  ...
  \item \textbf{Encoder-only and discriminative models.} A masked language model
        ...
        develops. This book is about systems whose primary output is open-ended
...
\section{What the system computes}
\label{sec:what-computed}

The computation has four stages, and the first is where the
canonical-form argument originates.

\paragraph{Tokenization.} Text is segmented into tokens drawn from a fixed
...
\texttt{Q1 2024} and \texttt{first quarter of 2024}. No semantic normalization
...
feed-forward transformation, with residual connections and normalization.

\paragraph{Unembedding and factorization.} The final layer is projected to logits
over $\mathcal{V}$ and normalized, giving $P(x_k \mid x_{<k}; W)$, where $W$ denotes
the fitted model parameters --- distinct from $\Theta$, which throughout this book is
the set of inference configurations, and from $\theta \in \Theta$, one such
configuration. The causal
mask is what licenses reading the whole as an autoregressive factorization:
\begin{equation}
  P(x_1, \ldots, x_n; W) = \prod_{k=1}^{n} P(x_k \mid x_{<k}; W).
  \label{eq:factorization}
\end{equation}

Now state the signature of the resulting map plainly, because everything that follows
about canonical form rests on it:
\begin{equation}
...
decisions, not a map from strings to strings. Everything this book asks for lives in the
difference between the two. The domain
is $\mathcal{V}^{*}$. Not propositions, not normalized queries, not parsed
intents, not records with identity---token sequences. There is no slot in the
parameterization for a proposition, and no stage between the tokenizer and the
...
distinction worth keeping, and this is not an instance of the canonical-form
failure. What it shows is narrower than an
absence of internal representation, and the narrower claim is the one this book needs.
...
\section{What the model returns is a distribution, not an answer}
\label{sec:sampling}

One property of \eqref{eq:factorization} is easy to pass over because it is so
familiar, and it carries more weight in what follows than any other single fact
about these systems. The model does not compute an output. It computes a
distribution over next tokens, and a separate decoding step turns that distribution
into text.

Where the distribution is sampled --- the ordinary configuration for open-ended
generation --- the same request can produce different text on different occasions.
Temperature and nucleus parameters shape how much the output varies; they are knobs
on the variance, not a switch that removes it. This is not a defect in a particular
product, and it is not a misconfiguration. It is what a probability distribution
over sequences is for. A system asked to generate fluent prose that never varies
would be a worse generator, which is why no vendor ships one.

Two consequences follow, and both are load-bearing later.

The first is that a verified output is a statement about one draw. The check
established what came back that time. It did not establish what the process will
return the next time the same question is asked, because there was no single thing
the process was going to return.

The second is that even holding the request fixed, the conditioning does not hold
still. A session accumulates context, and \eqref{eq:factorization} conditions on
everything in the window. The characters a user types may be identical on the second
occasion while the sequence the model conditions on is not. Greedy decoding narrows
the first consequence considerably and does not remove the second, and it does not
by itself deliver identical output run to run either --- for reasons of accelerator
arithmetic and request batching that are taken up where the measurement is
specified.

None of this makes these systems unusable, and none of it is exotic. Plenty of
non-deterministic computation is relied upon in regulated settings. It does mean
that any assurance argument of the form \emph{we tested it and it passed} has to say
what it thinks the test established, and that question turns out to be harder to
answer than it looks.

\section{The pretraining objective}
\label{sec:pretraining}

Parameters are fitted by minimizing cross-entropy on next-token prediction over a
large corpus:
\begin{equation}
  \mathcal{L}_{\mathrm{pre}}
  = -\sum_{k} \log P\!\left(x_k \mid x_{<k}; W\right).
  \label{eq:objective}
\end{equation}
...
term that is minimized by declining to produce a continuation---the objective is
...
five failure categories set out in the next chapter. Off the training
...
is a separate matter, and the distinction
between those two outcomes is far less important than it looks.

\section{In-context learning, and why the prompt is the whole control surface}
\label{sec:icl}

The property of this generation of models that made enterprise use plausible was
not raw perplexity. It was in-context learning: task behavior induced by
...
specific business task without a training program per task, and it is why these
systems entered production at all.

It also has a consequence that is easy to miss and directly relevant to
governance. If task behavior is induced through the prompt, then the prompt is the
...
This is the architectural root of the canonical-form failure. Other channels exist---system prompts,
...
\section{Instruction tuning and preference optimization}
\label{sec:rlhf}
...
optimization: a reward model $r_\phi$ is fitted to human comparisons between
candidate outputs, and the policy is optimized against $r_\phi$ with a divergence
penalty against the supervised reference,
\begin{equation}
  \max_{W}\ \mathbb{E}\bigl[r_\phi(x, y)\bigr]
  - \beta\, \mathbb{D}_{\mathrm{KL}}\!\left(\pi_W(y \mid x)
    \,\|\, \pi_{\mathrm{ref}}(y \mid x)\right).
  \label{eq:rlhf}
\end{equation}

Here $x$ is the prompt as assembled and sent to the model, $y$ a candidate output
for it, $\pi_W$ the policy being trained and $\pi_{\mathrm{ref}}$ the supervised
reference it is held near, $\mathbb{D}_{\mathrm{KL}}$ the Kullback--Leibler
divergence between them, and $\beta$ the weight on that penalty. $y$ rather than $o$
is used because the objective ranges over candidate outputs being compared, not over
the one output finally delivered.
...
book, three properties of \eqref{eq:rlhf} matter.
...
is the evidential base $E$ --- the identified, versioned sources an assertion is
supposed to rest on --- nor the source version, nor the resolved context. An assertion
...
false. Preference optimization \emph{can} select for the observable correlates of
...
to check at labeling time---which does not generalize to evidence they never saw.
...
is conditional. \emph{Preference optimization alone does not establish a
...
sycophancy results are a measured
...
about whether citations checked out on the examples that were labeled, and cannot
carry one about evidence nobody looked at. So the conventions generalize and the
...
\section{Three components that are absent}
\label{sec:missing}
...
resolution step, and an abstention branch---indeed the
invariants stated later require all three. The point is that none of them comes from the
...
base, forms a judgment about whether it supports the candidate claim, and
...
assertion remains available in retrieval-augmented systems, and why the requirement that
an assertion be \emph{sensitive} to its evidence --- that it would not have been made
had the evidence been absent --- is a genuine constraint rather than a description of
existing behavior.
...
mechanism whose function is to notice the omission. This is the failure developed
later under the name misapplied evidence.
...
abstention behavior is sensitive to phrasing: it is an output, not a branch. Any
system that must reliably decline needs the decision to sit outside $M$.

\section{A floor, not merely a gap}
\label{sec:floor}
...
% submitting. The characterization below is from the result as I understand it and the
...
\section{What this predicts about better models}
...
before. The next chapter gives these four outcomes
names and assigns them owners.
...
something structurally similar to the external gate this book argues for, and it
would be wrong to claim the mechanism is
...
computation over the assertion. It is settled by evidence and by
authority. So
outcome verification is a real mechanism that addresses a class of problems this book
...
remains $\mathcal{V}^{*}$; there is still no canonicalization stage; the reward
still cannot see provenance. Retrieval is the most consequential of them for this
book's purposes, and its contribution is not primarily accuracy---it is that an
identified evidential base is what makes the grounding rate observable at all.

\section{How this compares with other model families}
...
and predictive models are unproblematic, and the overlap with this book's categories
...
than the pathology, the formatting artifact rather than the content---is right for
the wrong reason. That is the same defect this book calls a \emph{lucky} assertion:
...
shift has exactly the problem of a sample that does not generalize. And per-instance
