...
\textbf{Critical distinction:} A classifier score below threshold is NOT a ``pass'' in the same sense as a deterministic evaluation pass. It means ``the classifier did not reach the threshold of suspicion at the configured sensitivity.'' The audit record must use quantized satisfaction notation --- \texttt{o $\vDash$\_\allowbreak{}$\tau$ c} --- meaning ``output o satisfied constraint c at threshold $\tau$.'' This is weaker evidence than deterministic satisfaction (\texttt{o $\vDash$ c}) and the audit record must make this distinction explicit. Calling a below-threshold score a ``pass'' is regulatory dishonesty.
