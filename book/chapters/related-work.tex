...
This book draws on nine bodies of work and departs from each in the same way: it
...
contribution is integration and operationalization, not discovery in any of these
...
summarization \citep{maynez2020} and is established as standard by surveys of the area
\citep{ji2023}; this book claims no priority for it. \citet{liu2023} measure how
...
\paragraph{Data quality dimensions, evidence doctrine, and records management.}
Six of the seven dimensions of Section~\ref{sec:six-dimensions} are established
terms of art, and Section~\ref{sec:six-dimensions} attests them individually. Three
vocabularies supply them. The data quality literature has provided dimensional
frameworks for three decades --- believability, reputation, relevancy,
completeness, currency and the rest --- codified in enterprise data management
bodies of knowledge and in the corresponding ISO standards. Evidence doctrine
supplies authentication, relevance and materiality, the best-evidence preference,
weight, sufficiency, and chain of custody. Records management and archival science
supply authenticity, reliability, integrity and usability as the qualities a record
must have to be relied upon; evidence-grading frameworks in medicine supply
indirectness, which is applicability under another name.

The contribution is the partition rather than the vocabulary. Data quality
dimensions are assessed by profiling a corpus and are properties of it. Evidence
doctrine is settled by a tribunal, after the fact, about an item offered for a
purpose. Records management governs the artifact, not the inference drawn from it.
None of the three is evaluated at the moment a machine emits a sentence, which is
the only moment at which a control can act --- and reorganizing the dimensions
around that moment is what separates document properties from relations to a
resolved context, and shows the second group to be where retrieval-based systems
fail and where corpus-level assurance cannot reach.

% TODO(cite): Wang and Strong's data quality dimensional framework (1996) and the
% enterprise data management and ISO data quality standards that codify it; the
% rules of evidence on authentication, relevance and materiality; ISO 15489 and the
% archival-science literature on record authenticity and reliability; and the GRADE
% treatment of indirectness. Named here without citation keys because I have not
% verified editions, clause numbers, or rule numbers, and a wrong rule number in
% this paragraph would be worse than none.
...
this book leans on in Section~\ref{sec:resolved-context} is standard bitemporal
modeling \citep{snodgrass1999}, and none of it is offered as novel.

What this book does is apply those requirements to machine-generated assertion, where
...
The departure is the one recorded at I5 and generalized in
...
\paragraph{Robustness, paraphrase consistency, and behavioral testing.} The
observation that model behavior moves under semantically inert input variation is
...
\citep{sharma2024}. Behavioral and capability-based testing beyond aggregate accuracy
is the program of \citet{ribeiro2020}, and the form-invariance audit of
Section~\ref{sec:form-audit} is a specialization of it.

The departure is what the instability is taken to \emph{mean}. That literature treats
paraphrase sensitivity primarily as a robustness defect to be reduced. This book
...
book indexes into $c$ are each recognized in their own literatures --- temporal
...
controlled design this book requires and is explicit that the protocol
...
\paragraph{Alignment and output shaping.} Preference optimization
\citep{ouyang2022} and rule-conditioned training \citep{bai2022} shape output
distributions and improve compliance behavior; Section~\ref{sec:rlhf} treats the
