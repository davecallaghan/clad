% !TEX root = ../main.tex
\chapter{Scale, Scope, and What Is Actually Tunable}
\label{ch:scale-and-scope}
\label{sec:scale}
...
\section{What scale does, and what it cannot do}
...
truth. Accuracy gains from scale are therefore gains in modeling the training
distribution, which correlates with truth to the degree the corpus is truthful. They
are not convergence toward truth as an optimization target, because no term in the
...
\section{Reproducibility is a property of topology, not of size}
...
\section{Corpus scope changes what can be measured}
...
Two of this book's measurements require establishing a \emph{negative}: that the
evidence does not support a claim. The abstention items of the
evaluation protocol need it by construction---an item whose correct
behavior is silence is only well formed if you can establish that the answer is
...
domain corpus you can characterize coverage and therefore estimate where the floor
...
\section{What the dials actually buy}
...
\section{The conclusion that does not follow}
...
restrictions on sub-processing. An organization that cannot obtain those has not
...
the one place in this book where the binding control is legal rather than technical.
...
constraint. So an organization that responds to a grounding problem by buying a larger
