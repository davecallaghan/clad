% !TEX root = ../main.tex
\chapter{A Validation Benchmark: Corpus and Case Design}
\label{sec:benchmark}

The chapter on limitations concedes that the framework's claims are unmeasured. This
...
\section{Why a real corpus rather than a synthetic one}
...
EMA medicines register
  & EU authorized medicines: therapeutic indication, active substance, revision number,
    authorization, withdrawal, suspension dates
  & Register is of \emph{centrally} authorized medicines, so coverage skews toward
    biologics and specialist products \\
\bottomrule
\end{tabular}
\end{center}

The division of labor between the first two is forced rather than chosen: openFDA
...
\begin{enumerate}
  ...
  \item \textbf{Jurisdiction.} A question answered from a US label where the EU
        authorization for the same active substance carries a different indication, or
...
\section{Configurations and measures}
...
\section{Reuse conditions}

US federal sources are public domain as works of the United States government. The EMA
download page states no license or reuse terms, and that should be resolved before
