...
Concretely: g$_1$ $\oplus$ g$_2$ governs S$_1$ $\cup$ S$_2$, so (g$_1$ $\oplus$ g$_2$) $\oplus$ g$_1$ has overlapping surfaces and is undefined. Formally, $\oplus$ is a total function into an error-or-result sum: g$_1$ $\oplus$ g$_2$ yields either a composed component (when S\_\allowbreak{}g$_1$ $\cap$ S\_\allowbreak{}g$_2$ = $\emptyset$) or a ComposeError carrying the overlapping surfaces --- matching the Lean (\texttt{Except ComposeError ComponentSpec}) and Scala (\texttt{Either[ComposeError, ComponentSpec]}) implementations. This is a partial commutative monoid --- a real structure, and the right one, but not a monoid.
