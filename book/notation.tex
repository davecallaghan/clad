...
% The five measurement macros below were carried over from an earlier draft's
% evaluation protocol and are not referenced anywhere in the book. They are
% retained as the declared vocabulary if that section is expanded; delete them
% freely if the slot is needed.
...
% Note: the CLAD chapters write \vDash and \succ directly rather than through
% \satisfies and \governs, matching the first half's convention. The macros are
% retained as semantic aliases if a future pass unifies them.
